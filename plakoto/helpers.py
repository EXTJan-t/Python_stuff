def possible_moves(dice_roll:list, state:list):
    ...
    start_pos = [x for x in point_state if x == 0]
    moves = []
    for pos in start_pos:
          for roll in dice_roll:
                if can_move(state, start_pos, roll):
                     moves.append((start_pos, roll))
    return moves
...
def can_move(state:list,start:int, roll:int):
    ...
    point_state, self_pieces, others_pieces = state[0], state[1], state[2]
    if 0 <= end <= 23:
        if point_state[end] == 0 or point_state[end] == -1 or sum(others_pieces[end], self_pieces[end]) == 1:
            return True
    elif end < 0:
